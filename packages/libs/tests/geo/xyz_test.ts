...
import { CRS84XYZ, XYZ } from "../../src/geo/xyz.ts";
...
Deno.test("Geo: Class CRS84XYZ", () => {
  const x = 104;
  const y = 20;
  const z = 7;
  const xyz = new CRS84XYZ(x, y, z);
  const lonlat = [113.90625, 32.34375];
  assertEquals(xyz.to_lonlat(), lonlat);
  assertEquals(xyz.to_ge_quadkey(), "02102033");
  assertEquals(
    CRS84XYZ.from_lonlat(114.686055, 31.707519, 10),
    new CRS84XYZ(838, 165, 10),
  );
});
